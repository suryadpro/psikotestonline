...
echo "Tanggal : ".$today;
if($_SESSION['level'] == 2) { $query2=mysql_query("select * from Hasil where nama like '%@$_SESSION[user_name]'"); } elseif($_SESSION['level'] == 1) { $query2=mysql_query("select * from Hasil"); } ?>
...
<br>
<form action="menu.php?data=monitoring&label=yes" method="POST">
By Label : <?php if($_SESSION['level'] == 1) { ?><select name="label"> <option></option> <?php $rumusan=mysql_query("select * from kebutuhan order by id desc"); while($rowe=mysql_fetch_array($rumusan)) { ?>
			<option value="<?php echo $rowe['id']; ?>" <?php if($_POST['label'] == $rowe['id'] ) { echo "selected"; } ?>><?php echo $rowe['nama']; ?></option> <?php } ?> </select> <?php }
			...
			<option value="<?php echo $rowe['id']; ?>" <?php if($_POST['label'] == $rowe['id'] ) { echo "selected"; } ?>><?php echo $rowe['nama']; ?></option> <?php } ?> </select> <?php } ?><input type="submit" value="cari"> </form>

<?php echo "Level : ".$_SESSION['level']; ?>
<form action="menu.php?data=monitoring&update=yes" method="POST">
...
$rumus="select * from Hasil where label='$_POST[label]'";}
...
$rumus="select * from Hasil where tanggal='$_POST[tanggal]'";}  }
else { $rumus="select * from Hasil where tanggal='$today'"; }
...
$log=mysql_query('insert into log_query values("","'.$_SESSION[user_name].'","'.$rumus.'")');
 $no=0; $ceke=mysql_num_rows($query); if ($ceke == 0) { ?> <tr> <td colspan=6 align="center"> Data yang dicari tidak ditemukan </td> <?php } else { $b=0; while($row=mysql_fetch_array($query)) { $b=$b+1; ?>
	<tr>
	 ...
	 <td> <?php if ($row[nama] == ' ') { $rumus=mysql_query("select nama_lengkap,kebutuhan from register where user_name='$row[user_name]'"); } else { $rumus=mysql_query("select nama_lengkap,kebutuhan,user_id from register where user_id='$row[user_name]'"); } $rowe=mysql_fetch_array($rumus); echo $rowe['nama_lengkap']."( "; $rumusan=mysql_query("select nama from kebutuhan where id='$rowe[kebutuhan]'"); $hile=mysql_fetch_array($rumusan); echo $hile['nama']." ) ";  ?> </td>
	 ...
	 <Td><input type="hidden" name="id<?php echo $b; ?>" value="<?php echo $row[user_name]; ?>"> <select name="status<?php echo $b; ?>"> <?php $rumus=mysql_query("select status from register where nama_lengkap='$rowe[nama_lengkap]'"); $array=mysql_fetch_array($rumus); ?>
...
<?php if($_GET['label'] == "yes" && $_SESSION['level'] == 1) { ?><a href="download_label.php?label=<?php echo $_POST['label']; ?>"> Download all </a> <?php } ?>
</body>
</html>