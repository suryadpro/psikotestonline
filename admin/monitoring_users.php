...
echo "Tanggal : ".$today;
if($_SESSION['level'] == 1 || $_SESSION['level'] == 2) { $query2=mysql_query("select * from hasil a,register b where a.user_id=b.user_id"); } ?>
<form action="menu.php?data=monitoring_usersusers&tanggal=on" method="POST" class="pure-form">
...
<form action="menu.php?data=monitoring_users&proses=yes" method="POST" class="pure-form">
<br>
<hr>
<br>
...
<hr>
<br>
<form action="menu.php?data=monitoring_users&label=yes" method="POST" class="pure-form">
By Label : <?php if($_SESSION['level'] == 1 or $_SESSION['level'] == 2) { ?><select name="label"> <option></option> <?php $rumusan=mysql_query("
...
<hr>
<br>
<form action="menu.php?data=monitoring_users&bulan=yes" method="post" class="pure-form">			
			per Bulan :
...
else { $rumus="select *,a.user_id as user from register a,hasil b  where a.user_id=b.user_id and b.tanggal='$today'"; }
...
$query=mysql_query($rumus);
 $no=0; $ceke=mysql_num_rows($query); if ($ceke == 0) { ?> <tr> <td colspan=12 align="center"> Data yang dicari tidak ditemukan </td> <?php } else { $b=0; while($row=mysql_fetch_array($query)) { $b=$b+1; ?>
	...
	<tr>
	 <td> <input type="checkbox" name="idu<?php echo $no; ?>" value="<?php echo $row['user_id']; ?>">  </td>
	 <td> <?php echo $no; ?> </td>
	...
	switch($row2['status']) { case 0 : echo "-"; break; case 1 : echo "Lulus"; break; case 2 : echo "Tidak Lulus"; break; case 3 : echo "Tidak Hadir"; break;} ?></td>
	 <td> <?php if($_SESSION['level'] == 1) { ?><a href="downloaded.php?user_name=<?php echo $row['user_name']; ?>"> Download jawaban psikotest </a> |
			<?php $ru=mysql_query("select * from papi_final where id_user='$row[user_id]'"); $con=mysql_num_rows($ru); if($array['status'] == "downloaded" || $array['status'] == "done") { if($con == 0) { ?><a href="burn_papi.php?id=<?php echo $row['user_id']; ?>"> Buat hasil papi </a> <?php } else { ?><a href="downloaded2.php?user_name=<?php echo $row['user_id']; ?>"> Download hasil psikogram </a> | <a href="downloaded3.php?user_name=<?php echo $row['user_id']; ?>"> Download hasil psikogram 2</a> <?php }}else{echo "download psikogram";}} ?></td>
	</tr> <?php }} ?>
</table>
<br>
