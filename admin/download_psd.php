...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <?php $query3=mysql_query("select * from register where user_name like '%@psd.jkt'"); while ($row5=mysql_fetch_array($query3)) { ?>
 <Worksheet ss:Name="<?php echo $row5['user_name']; ?>">
  <Table>
   ...
   </Row> <?php $query2=mysql_query("select * from register where user_id='$row5[user_id]'"); $row3=mysql_fetch_array($query2); $query=mysql_query("select * from jawaban where user_name='$row3[user_name]'"); $cek=mysql_num_rows($query); if ($cek == 0) { $query=mysql_query("select * from jawaban where user_name='$row3[user_id]'"); } while($row=mysql_fetch_array($query)) { ?>
   <Row> 
    <Cell ss:StyleID="s62"><Data ss:Type="String"><?php  echo $row3['nama_lengkap'];  ?></Data></Cell>
